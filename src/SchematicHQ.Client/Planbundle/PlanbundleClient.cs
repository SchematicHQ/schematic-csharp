...
public partial class PlanbundleClient : IPlanbundleClient
{
    ...
    private async Task<
        WithRawResponse<CreateCustomPlanBundleResponse>
    > CreateCustomPlanBundleAsyncCore(
    ...
    )
    {
        ...
        if (response.StatusCode is >= 200 and < 400)
        {
            ...
            try
            {
                var responseData = JsonUtils.Deserialize<CreateCustomPlanBundleResponse>(
                    responseBody
                )!;
                return new WithRawResponse<CreateCustomPlanBundleResponse>()
                {
                    Data = responseData,
    ...
    /// );
    /// </code></example>
    public WithRawResponseTask<CreateCustomPlanBundleResponse> CreateCustomPlanBundleAsync(
        CreateCustomPlanBundleRequestBody request,
        RequestOptions? options = null,
        CancellationToken cancellationToken = default
    )
    {
        return new WithRawResponseTask<CreateCustomPlanBundleResponse>(
            CreateCustomPlanBundleAsyncCore(request, options, cancellationToken)
        );
    }
